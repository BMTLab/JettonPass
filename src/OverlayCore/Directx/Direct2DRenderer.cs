...
using static JettonPass.OverlayCore.Common.NativeMethods;
...
namespace JettonPass.OverlayCore.Directx
{
    ...
    public sealed class Direct2DRenderer
    {
        ...
        ///     The transparent
        /// </summary>
        private static readonly RawColor4 Transparent = new(
            GdiTransparent.R,
            GdiTransparent.G,
            GdiTransparent.B,
            GdiTransparent.A
        ...
        ///     The brush container
        /// </summary>
        private List<SolidColorBrush> _brushContainer = new(16);
        ...
        ///     The font container
        /// </summary>
        private List<TextFormat> _fontContainer = new(16);

        /// <summary>
        ///     The layout container
        /// </summary>
        private List<TextLayoutBuffer> _layoutContainer = new(16);
        ...
        public void Dispose()
        {
            ...
            _brushContainer.Clear();
            _fontContainer.Clear();
            _layoutContainer.Clear();
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void DrawLine(int startX, int startY, int endX, int endY, float stroke, int brush) =>
        ...
        /// <param name="stroke">The stroke.</param>
        /// <param name="brush">The brush.</param>
        public void DrawEdge(int x, int y, int width, int height, int length, float stroke, int brush)
        {
        ...
        public void FillRectangle(int x, int y, int width, int height, int brush) =>
            _device.FillRectangle(new RawRectangleF(x, y, x + width, y + height), _brushContainer[brush]);
        ...
        public void FillCircle(int x, int y, int radius, int brush) =>
            _device.FillEllipse(new Ellipse(new RawVector2(x, y), radius, radius), _brushContainer[brush]);
        ...
        public void BorderedLine(int startX, int startY, int endX, int endY, float stroke, int brush, int borderBrush)
        {
            ...
            _device.DrawLine
            (
                new RawVector2(startX, startY - stroke),
                new RawVector2(endX, endY - stroke),
                _brushContainer[borderBrush],
                stroke
            );
            ...
            _device.DrawLine
            (
                new RawVector2(startX - stroke / 2, startY - stroke * 1.5f),
                new RawVector2(startX - stroke / 2, startY + stroke * 1.5f),
                _brushContainer[borderBrush],
                stroke
            );
            ...
            _device.DrawLine
            (
                new RawVector2(endX - stroke / 2, endY - stroke * 1.5f),
                new RawVector2(endX - stroke / 2, endY + stroke * 1.5f),
                _brushContainer[borderBrush],
                stroke
            );
        ...
        public void DrawText(string text, int font, int brush, int x, int y, bool bufferText = true)
        {
            if (bufferText)
            {
                ...
                if (bufferPos == -1)
                {
                    _layoutContainer.Add
                    (
                        new TextLayoutBuffer
                        (
                            text,
                ...
                _device.DrawTextLayout
                (
                    new RawVector2(x, y),
                    _layoutContainer[bufferPos].TextLayout,
                    _brushContainer[brush],
